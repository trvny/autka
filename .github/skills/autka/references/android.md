...
- **Stack (verify against `gradle/libs.versions.toml` — it drifts; figures below confirmed live 2026-06-28):** AGP 9.2.1, Kotlin 2.4.0, KSP 2.3.9, Compose BOM 2026.06.00, Hilt 2.59.2, Room 2.8.4, coroutines 1.11.0, DataStore 1.2.1, Navigation 2.9.8, Retrofit 3.0.0 + okhttp 5.4.0 + kotlinx-serialization, Coil 2.7.0, **osmdroid 6.1.20** (maps), **compileSdk 37**, targetSdk 35 (→36 in flight, PR #62), minSdk 26, JVM 17. **The AGP 9 `kotlinOptions` migration is already done** — compiler options live in a top-level `kotlin { compilerOptions { jvmTarget = JvmTarget.JVM_17 } }` block in `app/build.gradle.kts`; don't reintroduce `android.kotlinOptions`. **Watch the Hilt/Kotlin-metadata gotcha:** the build force-pins `org.jetbrains.kotlin:kotlin-metadata-jvm:2.4.0` because Hilt 2.59.2's aggregating step only reads metadata ≤2.3.0 otherwise — keep that `resolutionStrategy.force` until Hilt bundles a newer reader. Bump versions only via the catalog; KSP/Hilt/AGP are matched to Kotlin, so if you move one, check the others.
- **Pending upgrade flags (confirmed against the live catalog 2026-06-28):**
  - **targetSdk 35 → 36 — hard deadline, in flight (PR #62).** Google Play requires **targetSdk 36 (Android 16) for all new apps and updates from 2026-08-31**; below that the app stops being discoverable to users on newer Android versions (existing installs keep working). compileSdk is already 37. This is the one item with a fixed external clock.
  - **Coil 2.7.0 → 3.x is the only remaining library migration** — a major-version jump with breaking API changes (new `coil3` artifact coordinates, `AsyncImage` API shifts); treat as a deliberate migration, not a routine catalog bump.
  - **Already done (don't re-flag):** AGP 9 + the `kotlin {}` compiler-options migration, Compose BOM 2026.06.00, Retrofit 3.0.0, compileSdk 37 are all landed. The earlier "Compose/Retrofit/AGP pending" notes are stale.
  - **AGP 10 horizon:** the legacy `android.kotlinOptions` escape hatch is slated for complete removal in **AGP 10 (~H2 2026)**; the top-level `kotlin { compilerOptions {} }` block (already in use here) is the forward-compatible form.
